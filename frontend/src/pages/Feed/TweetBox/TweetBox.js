...
const TweetBox = () => {
    ...
    useEffect(() => {
        const fetchPostRemains = async () => {
            if (email) {
                try {
                    const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/user/postRemains`, {
                        params: { email: email }
                    });
                    if (response.data) {
    ...
    const handleTweet = async (e) => {
        e.preventDefault();
        if (postRemains <= 0) {
            showSnackbar('You have reached your posting limit.', 'warning');
            return;
        }
        await submitPost();
    };
    ...
    const sendOtp = async () => {
        try {
            const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/otp`, { email: email });
    ...
    const checkVideo = async () => {
        try {
            ...
            if (video) {
                ...
                // Just get the url if it is not video file.
                getURL();
            }
        } catch (error) {
            setIsLoading(false);
    ...
    const uploadFile = async (type) => {
        if ((type === 'image' && !image) || (type === 'video' && !video)) {
            return;
        }
    ...
    const submitPost = async () => {
        if (loggedInUser[0]?.email) {
            ...
            try {
                await axios.post(`${process.env.REACT_APP_BACKEND_URL}/post`, userPost);
                await axios.post(`${process.env.REACT_APP_BACKEND_URL}/user/decrementPostRemains`, { email: email });
        ...
        <div className="tweetBox">
            ...
            >
                ...
                <DialogActions>
                    ...
                    <Button onClick={checkVideo}><UploadIcon /> Upload</Button>
                </DialogActions>
            </Dialog>
