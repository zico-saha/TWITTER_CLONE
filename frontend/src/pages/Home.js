...
const Home = () => {
    ...
    const checkPlanExpiry = async () => {
        try {
            ...
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/user?email=${user.email}`);
    ...
    const downgradePlan = async () => {
        try {
            ...
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/downgradePlan`, {
                method: 'POST',
                headers: {
